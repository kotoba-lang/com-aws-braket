(ns aws_braket.main
  "Kotodama WASM entrypoint for the AWS Braket clean-room actor (L4) — Clojure port.
  ...
  facts (`aws_braket.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "aws_braket")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"aws_braket.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "aws_braket-compat" :tier tier :entities entities} 200])
...
(defn start! [] :aws_braket-compat/ready)
